...
module OCI
  ...
  class OsManagementHub::WorkRequestClient
    ...
    def list_work_requests(opts = {})
      ...
      status_allowable_values = %w[WAITING ACCEPTED IN_PROGRESS FAILED SUCCEEDED CANCELING CANCELED SKIPPED]
      if opts[:status] && !opts[:status].empty?
        opts[:status].each do |val_to_check|
          unless status_allowable_values.include?(val_to_check)
            raise 'Invalid value for "status", must be one of WAITING, ACCEPTED, IN_PROGRESS, FAILED, SUCCEEDED, CANCELING, CANCELED, SKIPPED.'
          end
        end
      end
