...
module OCI
  ...
  class ApmTraces::TraceClient
    ...
    #
    # @param [DateTime] time_log_started_greater_than_or_equal_to Include logs with log time equal to or greater than this value.
    #
    # @param [DateTime] time_log_ended_less_than Include logs with log time less than this value.
    #
    # @param [Hash] opts the optional parameters
    # @option opts [OCI::Retry::RetryConfig] :retry_config The retry configuration to apply to this operation. If no key is provided then the service-level
    #   retry configuration defined by {#retry_config} will be used. If an explicit `nil` value is provided then the operation will not retry
    ...
    #   might be rejected.
    #
    # @return [Response] A Response object with data of type {OCI::ApmTraces::Models::Log Log}
    # @note Click [here](https://docs.cloud.oracle.com/en-us/iaas/tools/ruby-sdk-examples/latest/apmtraces/get_log.rb.html) to see an example of how to use get_log API.
    def get_log(apm_domain_id, log_key, time_log_started_greater_than_or_equal_to, time_log_ended_less_than, opts = {})
      logger.debug 'Calling operation TraceClient#get_log.' if logger

      raise "Missing the required parameter 'apm_domain_id' when calling get_log." if apm_domain_id.nil?
      raise "Missing the required parameter 'log_key' when calling get_log." if log_key.nil?
      raise "Missing the required parameter 'time_log_started_greater_than_or_equal_to' when calling get_log." if time_log_started_greater_than_or_equal_to.nil?
      raise "Missing the required parameter 'time_log_ended_less_than' when calling get_log." if time_log_ended_less_than.nil?
      raise "Parameter value for 'log_key' must not be blank" if OCI::Internal::Util.blank_string?(log_key)

      path = '/logs/{logKey}'.sub('{logKey}', log_key.to_s)
      operation_signing_strategy = :standard
      ...
      query_params = {}
      query_params[:apmDomainId] = apm_domain_id
      query_params[:timeLogStartedGreaterThanOrEqualTo] = time_log_started_greater_than_or_equal_to
      query_params[:timeLogEndedLessThan] = time_log_ended_less_than
      ...
      # rubocop:disable Metrics/BlockLength
      OCI::Retry.make_retrying_call(applicable_retry_config(opts), call_name: 'TraceClient#get_log') do
        @api_client.call_api(
          :GET,
          path,
          endpoint,
          ...
          operation_signing_strategy: operation_signing_strategy,
          body: post_body,
          return_type: 'OCI::ApmTraces::Models::Log'
        )
      end
      # rubocop:enable Metrics/BlockLength
    end
