...
module OCI
  ...
  class Adm::ApplicationDependencyManagementClient
    ...
    # @option opts [OCI::Retry::RetryConfig] :retry_config The retry configuration to apply to this operation. If no key is provided then the service-level
    #   retry configuration defined by {#retry_config} will be used. If an explicit `nil` value is provided then the operation will not retry
    # @option opts [BOOLEAN] :is_direct_vulnerability A filter when set returns only vulnerabilities that are immediately(directly) affecting any artifact, when unset all vulnerabilities are returned.
    # @option opts [Integer] :limit The maximum number of items to return. (default to 100)
    # @option opts [String] :page A token representing the position at which to start retrieving results. This must come from the `opc-next-page` header field of a previous response.
    # @option opts [String] :opc_request_id The client request ID for tracing.
    # @return [Response] A Response object with data of type {OCI::Adm::Models::VulnerabilityDetailsCollection VulnerabilityDetailsCollection}
    # @note Click [here](https://docs.cloud.oracle.com/en-us/iaas/tools/ruby-sdk-examples/latest/adm/list_vulnerabilities.rb.html) to see an example of how to use list_vulnerabilities API.
    def list_vulnerabilities(vulnerability_audit_id, opts = {})
      logger.debug 'Calling operation ApplicationDependencyManagementClient#list_vulnerabilities.' if logger

      raise "Missing the required parameter 'vulnerability_audit_id' when calling list_vulnerabilities." if vulnerability_audit_id.nil?
      raise "Parameter value for 'vulnerability_audit_id' must not be blank" if OCI::Internal::Util.blank_string?(vulnerability_audit_id)

      path = '/vulnerabilityAudits/{vulnerabilityAuditId}/vulnerabilities'.sub('{vulnerabilityAuditId}', vulnerability_audit_id.to_s)
      operation_signing_strategy = :standard

      # rubocop:disable Style/NegatedIf
      # Query Params
      query_params = {}
      query_params[:isDirectVulnerability] = opts[:is_direct_vulnerability] if !opts[:is_direct_vulnerability].nil?
      query_params[:limit] = opts[:limit] if opts[:limit]
      query_params[:page] = opts[:page] if opts[:page]
      ...
      # rubocop:disable Metrics/BlockLength
      OCI::Retry.make_retrying_call(applicable_retry_config(opts), call_name: 'ApplicationDependencyManagementClient#list_vulnerabilities') do
        @api_client.call_api(
          :GET,
          path,
          endpoint,
          ...
          operation_signing_strategy: operation_signing_strategy,
          body: post_body,
          return_type: 'OCI::Adm::Models::VulnerabilityDetailsCollection'
        )
      end
      # rubocop:enable Metrics/BlockLength
    end
    ...
    #   Required only if the id query param is not specified.
    #
    # @option opts [String] :knowledge_base_id A filter to return only Audits that were created against the specified knowledge base.
