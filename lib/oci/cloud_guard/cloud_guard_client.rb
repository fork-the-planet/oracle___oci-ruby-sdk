...
module OCI
  ...
  class CloudGuard::CloudGuardClient
    ...
    #   Allowed values are: RESOURCE_TYPE, REGION, COMPARTMENT_ID, RISK_LEVEL, DETECTOR_ID, TARGET_ID
    # @param [String] compartment_id The OCID of the compartment in which to list resources.
    # @param [Hash] opts the optional parameters
    # @option opts [OCI::Retry::RetryConfig] :retry_config The retry configuration to apply to this operation. If no key is provided then the service-level
    #   retry configuration defined by {#retry_config} will be used. If an explicit `nil` value is provided then the operation will not retry
    ...
    def request_summarized_problems(list_dimensions, compartment_id, opts = {})
      ...
      list_dimensions_allowable_values = %w[RESOURCE_TYPE REGION COMPARTMENT_ID RISK_LEVEL DETECTOR_ID TARGET_ID]
      list_dimensions.each do |val_to_check|
        unless list_dimensions_allowable_values.include?(val_to_check)
          raise "Invalid value for 'list_dimensions', must be one of RESOURCE_TYPE, REGION, COMPARTMENT_ID, RISK_LEVEL, DETECTOR_ID, TARGET_ID."
        end
      end
