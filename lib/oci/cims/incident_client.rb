...
module OCI
  ...
  class Cims::IncidentClient
    ...
    #
    # @param [String] incident_key Unique identifier for the support request.
    ...
    # @option opts [String] :name The user-friendly name of the support request type. (default to limit)
    # @option opts [String] :ocid User OCID for Oracle Identity Cloud Service (IDCS) users who also have a federated Oracle Cloud Infrastructure account.
    #   User OCID is mandatory for OCI Users and optional for Multicloud users.
    #
    # @option opts [String] :homeregion The region of the tenancy.
    ...
    # @option opts [String] :sort_by The key to use to sort the returned items. (default to dateUpdated)
    # @option opts [String] :sort_order The order to sort the results in. (default to ASC)
    # @option opts [DateTime] :time_updated_greater_than_or_equal_to Filter to return results updated only after the specified timestamp. Must be an RFC 3339 timestamp (e.g. 2025-12-07T17:42:54Z).
    ...
    # @param [String, IO] put_attachment_details File to be uploaded as attachment to the Service Request.
    # @param [String] incident_key Unique identifier for the support request.
    # @param [String] attachment_name The name of the file to attach to the support request. Avoid entering confidential information.
    # @param [String] compartment_id The OCID of the tenancy.
    # @param [BOOLEAN] is_restricted_flag Set to `true` when the attachment contains personal information (PI)
    ...
    #
    # @return [Response] A Response object with data of type {OCI::Cims::Models::Incident Incident}
    # @note Click [here](https://docs.cloud.oracle.com/en-us/iaas/tools/ruby-sdk-examples/latest/cims/put_attachment.rb.html) to see an example of how to use put_attachment API.
    def put_attachment(put_attachment_details, incident_key, attachment_name, compartment_id, is_restricted_flag, opts = {})
      logger.debug 'Calling operation IncidentClient#put_attachment.' if logger

      raise "Missing the required parameter 'put_attachment_details' when calling put_attachment." if put_attachment_details.nil?
      raise "Missing the required parameter 'incident_key' when calling put_attachment." if incident_key.nil?
      raise "Missing the required parameter 'attachment_name' when calling put_attachment." if attachment_name.nil?
      raise "Missing the required parameter 'compartment_id' when calling put_attachment." if compartment_id.nil?
      raise "Missing the required parameter 'is_restricted_flag' when calling put_attachment." if is_restricted_flag.nil?
      raise "Parameter value for 'incident_key' must not be blank" if OCI::Internal::Util.blank_string?(incident_key)

      path = '/v2/incidents/{incidentKey}/attachment'.sub('{incidentKey}', incident_key.to_s)
      operation_signing_strategy = :standard

      # rubocop:disable Style/NegatedIf
      # Query Params
      query_params = {}
      query_params[:attachmentName] = attachment_name
      query_params[:compartmentId] = compartment_id
      query_params[:isRestrictedFlag] = is_restricted_flag
      ...
      post_body = @api_client.object_to_http_body(put_attachment_details)

      # rubocop:disable Metrics/BlockLength
      OCI::Retry.make_retrying_call(applicable_retry_config(opts), call_name: 'IncidentClient#put_attachment') do
        @api_client.call_api(
          :PUT,
          path,
          endpoint,
    ...
    #
    # @param [String] incident_key Unique identifier for the support request.
    # @param [OCI::Cims::Models::UpdateIncident] update_incident_details Details about the support request being updated.
    # @param [String] compartment_id The OCID of the tenancy.
    # @param [Hash] opts the optional parameters
    # @option opts [OCI::Retry::RetryConfig] :retry_config The retry configuration to apply to this operation. If no key is provided then the service-level
    #   retry configuration defined by {#retry_config} will be used. If an explicit `nil` value is provided then the operation will not retry
