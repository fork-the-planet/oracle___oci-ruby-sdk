...
module OCI
  ...
  class Devops::DevopsClient
    ...
    #
    # @option opts [BOOLEAN] :paths_in_subtree Flag to determine if files must be retrieved recursively. Flag is False by default.
    ...
    def list_refs(repository_id, opts = {})
      ...
      if opts[:sort_by] && !%w[refType refName timeUpdated].include?(opts[:sort_by])
        raise 'Invalid value for "sort_by", must be one of refType, refName, timeUpdated.'
      end
      raise "Parameter value for 'repository_id' must not be blank" if OCI::Internal::Util.blank_string?(repository_id)
    ...
    # @param [OCI::Devops::Models::TriggerDeploymentDryRunDetails] trigger_deployment_dry_run_details Details for dry run for the new Helm Deployment.
    # @param [Hash] opts the optional parameters
    # @option opts [OCI::Retry::RetryConfig] :retry_config The retry configuration to apply to this operation. If no key is provided then the service-level
    #   retry configuration defined by {#retry_config} will be used. If an explicit `nil` value is provided then the operation will not retry
    # @option opts [String] :opc_retry_token A token that uniquely identifies a request so it can be retried in case of a timeout or server error without risk of executing that same action again. Retry tokens expire after 24 hours, but can be invalidated earlier due to conflicting operations. For example, if a resource has been deleted and purged from the system, then a retry of the original creation request might be rejected.
    # @option opts [String] :opc_request_id Unique Oracle-assigned identifier for the request.  If you need to contact Oracle about a particular request, provide the request ID.
    # @return [Response] A Response object with data of type {OCI::Devops::Models::TriggerDryRunResult TriggerDryRunResult}
    # @note Click [here](https://docs.cloud.oracle.com/en-us/iaas/tools/ruby-sdk-examples/latest/devops/trigger_deployment_dry_run.rb.html) to see an example of how to use trigger_deployment_dry_run API.
    def trigger_deployment_dry_run(trigger_deployment_dry_run_details, opts = {})
      logger.debug 'Calling operation DevopsClient#trigger_deployment_dry_run.' if logger

      raise "Missing the required parameter 'trigger_deployment_dry_run_details' when calling trigger_deployment_dry_run." if trigger_deployment_dry_run_details.nil?
      ...
      post_body = @api_client.object_to_http_body(trigger_deployment_dry_run_details)

      # rubocop:disable Metrics/BlockLength
      OCI::Retry.make_retrying_call(applicable_retry_config(opts), call_name: 'DevopsClient#trigger_deployment_dry_run') do
        @api_client.call_api(
          :POST,
          path,
          endpoint,
          ...
          operation_signing_strategy: operation_signing_strategy,
          body: post_body,
          return_type: 'OCI::Devops::Models::TriggerDryRunResult'
        )
      end
      # rubocop:enable Metrics/BlockLength
    end
